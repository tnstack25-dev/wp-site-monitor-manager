...
<div class="wrap wpsmm-wrap wpsmm-logs-page">
    ...
    <section class="wpsmm-panel wpsmm-log-panel">
        <form method="get" action="<?php echo esc_url(admin_url('admin.php')); ?>" class="wpsmm-log-filters">
            ...
            <label class="wpsmm-log-search"><span class="dashicons dashicons-search"></span><input type="search" name="search" value="<?php echo esc_attr($filters['search']); ?>" placeholder="Tìm kiếm log..."></label>
        ...
        <div class="wpsmm-log-summary">
            <span>Tổng <b><?php echo esc_html(number_format_i18n($total)); ?></b> log</span>
        ...
        <div class="wpsmm-table-scroll">
            <table class="widefat wpsmm-log-table">
                <thead><tr><th>Thời gian</th><th>Mức độ</th><th>Loại log</th><th>Thông điệp</th><th>Website</th><th>Phản hồi</th><th>Chi tiết</th></tr></thead>
                <tbody>
                <?php if (!$logs): ?><tr><td colspan="7"><div class="wpsmm-empty-state">Không có log phù hợp với bộ lọc.</div></td></tr><?php endif; ?>
                <?php foreach ($logs as $log): ?>
                    ...
                    <tr>
                        ...
                        <td><button type="button" class="button button-small wpsmm-open-log" data-log="<?php echo esc_attr(wp_json_encode(['time' => $log->checked_at, 'site' => $log->site_name ?: '#' . $log->site_id, 'url' => $log->site_url, 'status' => $statusLabels[$log->status] ?? $log->status, 'http' => $log->http_code, 'response' => round((float) $log->response_time * 1000) . ' ms', 'message' => $log->message])); ?>">Xem</button></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
        ...
        <footer class="wpsmm-table-footer">
            <span>Hiển thị <?php echo esc_html(count($logs)); ?> trong tổng số <?php echo esc_html(number_format_i18n($total)); ?> log · Trang <?php echo esc_html($page); ?>/<?php echo esc_html($pages); ?></span>
    ...
    <div class="wpsmm-log-modal" id="wpsmm-log-modal" aria-hidden="true">
        ...
        <article class="wpsmm-log-modal-panel"><button type="button" class="wpsmm-log-modal-close" data-close-log aria-label="Đóng">&times;</button><h2>Chi tiết log</h2><dl id="wpsmm-log-detail"></dl></article>
    </div>
</div>
