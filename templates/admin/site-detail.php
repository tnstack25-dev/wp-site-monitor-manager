...
<div class="wrap wpsmm-wrap wpsmm-site-detail-page">
    ...
    <section class="wpsmm-detail-stats" id="wpsmm-detail-overview">
        ...
        <article><small>WP Site Monitor Agent</small><strong class="<?php echo ($site->agent_status ?? '') === 'online' ? 'wpsmm-text-success' : 'wpsmm-text-danger'; ?>"><?php echo esc_html(($site->agent_status ?? '') === 'online' ? 'Trực tuyến' : 'Ngoại tuyến'); ?></strong><em>Lần kiểm tra: <?php echo esc_html($site->agent_checked_at ?? 'chưa có dữ liệu'); ?></em></article>
    </section>

    <div class="wpsmm-detail-grid">
        <section class="wpsmm-panel wpsmm-detail-panel">
            <h2>Thông tin website</h2>
            <dl><div><dt>URL</dt><dd><a href="<?php echo esc_url($site->url); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html($site->url); ?></a></dd></div><div><dt>Đường dẫn kiểm tra trạng thái</dt><dd><?php echo esc_html($site->health_path ?: 'Trang chủ'); ?></dd></div><div><dt>Tự động giám sát</dt><dd><?php echo empty($site->monitor_enabled) ? 'Tạm dừng' : 'Đang bật'; ?></dd></div><div><dt>Nhóm</dt><dd><?php echo esc_html($site->group_name ?: 'Tất cả website'); ?></dd></div><div><dt>Mã HTTP</dt><dd><?php echo esc_html($site->http_code ?: '-'); ?></dd></div><div><dt>Mã HTTP mong đợi</dt><dd><?php echo esc_html($site->expected_status); ?></dd></div><div><dt>Tiêu đề mong đợi</dt><dd><?php echo esc_html($site->expected_title ?: 'Không kiểm tra'); ?></dd></div><div><dt>Ngày thêm</dt><dd><?php echo esc_html($site->created_at); ?></dd></div></dl>
        </section>
